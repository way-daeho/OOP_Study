...
// 		// 메모리 할당과 해제를 반복하지 않고, reserve를 해두는데, 메모리 할당 시, 현재 메모리 크기의 두 배 정도를 할당해둔다.
// 		// 새로 할당해야 할 메모리 크기(string_length + str.string_length)가 현재의 memory_capacity 의 두 배 이하라면,
// 		// 아예 memory_capacity의 두 배에 달하는 크기를 할당해버린다.

// 		// insert 되는 문자열의 크기가 엄청 커서 memory_capacity 의 두 배를 뛰어 넘어버린다면 그냥 예약을 생각하지 않고, 필요한 만큼 할당해버린다.
// 		// 해당 방법은 C++에서 동적으로 할당되는 메모리를 처리하는데 매우 빈번하게 사용되는 기법중 하나.
// 	}
...
// }
// MyString& MyString::insert(int loc, char c) { //-> MD 파일에 추가한다.
//   MyString temp(c);
//   return insert(loc, temp);
// }
...
/*
	erase 함수
	insert 함수의 정 반대의 역할
	기본적으로 데이터의 양이 줄어 드는 것이기 때문에 복잡하게 capacity를 생각할 필요가 없다.
...
/*
	find 함수
	insert 와 erase 이외 매우 빈번하게 사용되는 또 다른 작업
	insert, erase, find 는 문자열 연산의 기초 중의 기초
	insert와 erase는 연산 시간이 크게 오래 걸리지 않지만, 문자열의 크기가 매우 크다면 find 연산은 엄청 오래
	걸릴 수 있음.

	클래스 사용 목적에 따라 좋은 성능을 발휘하는 알고리즘을 만들어야함.
	MyString을 인자로 받는 find 만 제대로 구현한 후, 아래 두개의 	find는 맨 위의 함수를 이용, 구현하는 방식

	해당 함수 -> find_from 에서 부터 시작해서 가장 첫 번째 str의 위치를 리턴.
...
/*
	크기 비교 함수 'compare'
	문자열 간의 크기를 비교하는 함수
	'크기'를 비교한다는 의미 -> 사전식으로 배열해서 어떤 문자열이 더 뒤에 오는지 판단.
	C 언어에서 strcmp 함수 등으로 지원하였던 것들을 그대로 사용

	int compare(const MyString& str) const;

	int MyString::compare(const MyString& str) const {
		// (*this) - (str) 을 수행해서 그 1, 0, -1 로 그 결과를 리턴한다
		// 1은 (*this) 가 사전식으로 더 뒤에 온다는 의미. 0 은 두 문자열
		...
		// 참고로 abc 와 abcd 의 크기 비교는 abcd 가 더 위에 오게 되다.
...
/*
	explicit
	DoSomethinWithString(MyString("abc")) => 당연히 된다. MyString 객체를 생성해서 이를 인자로 전달.

	DoSomethinWithString("abc") "abc"는 MyString 타입이 아니다. C++ 컴파일러는 꽤나 똑똑하여,
	"abc"를 어떻게 하면 MyString으로 바꿀 수 있는지 생각.

	MyString(const char* str); 생성자가 존재.
	해당 생성자로 변환되서 컴파일 된다 이러한 과정을 암시적 변환(implicit conversion)이라 부른다.
	암시적 변환이 언제나 편리한 것은 아니다.

	원하지 않는 암시적 변환을 할 수 없도록 컴파일러에게 명시할 수 있다.

	explicit로 선언한다면, 해당 생성자가 복사 생성자의 형태로도 호출되는 것을 막게 된다.
	...
	명시적으로 생성자를 부를 때 에만 허용할 수 있게 된다.
*/

/*
	mutable (변이 가능한)
	const 멤버 함수의 경우, const 함수 내부에서는 멤버 변수들의 값을 바꾸는 것이 불가능하다.
	하지만, mutable로 선언하였다면, const 함수에서도 이들 값을 바꿀 수 있다.

	차라리 함수에서 const를 떼어버리는 게 낫지 않을까? 왜 mutable을 만들었을까.

	먼저 멤버 함수를 왜 const로 선언하는지 부터 생각해보자.
	클래스의 멤버 함수들은 이 객체는 이러이러한 일을 할 수 있습니다 라는 의미를 나타내고 있다.

	그리고 이 멤버 함수를 const 로 선언하는 의미는 "이 함수는 객체의 내부 상태에 영향을 주지 않는다." 를 표현하는 방법
	대표적인 예 -> 읽기 작업을 수행하는 함수
	서버를 예를 들면, 데이터베이스에 직접 접근하는 대신, 캐시 메모리로 접근
	캐시 메모리는 자주 접근하는 값을 저장해두는데, 그 값이 캐시 미스를 당하면 값을 업데이트 해야한다. 하지만 함수가 const형이라 변경 불가.
	하지만 캐시 변수를 mutable이라고 할 경우, 바꿔야할 경우를 대비해서 하면 캐시 메모리 값을 업데이트 할 수있다.
*/
// #include <iostream>
